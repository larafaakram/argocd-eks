pipeline {
    ...
    stages {
        ...
        stage('Clean Workspace') {
            steps { cleanWs() // Clean up previous files
            }
        }
        // Stage to checkout the code from Git
        stage('Git Checkout') {
            steps {
                git branch: 'main', credentialsId: 'git-cred', url: 'git@github.com:larafaakram/Multi-Tier-BankApp-CI.git'
            }
        }
        // Compile the code
        stage('Compile') {
                ...
                script {
                    withCredentials([gitUsernamePassword(credentialsId: 'git-cred', gitToolName: 'Default')]) {
                        sh '''
                            ...
                            sed -i 's|image: larafaakram/bankapp:.*|image: larafaakram/bankapp:'${DOCKER_TAG}'|' $(pwd)/bankapp/bankapp-ds.yml
                            '''
